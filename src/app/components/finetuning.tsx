...
export default function FineTuningClient() {
  ...
  const [yieldRate, setYieldRate] = useState<string>('12'); // APY in percentage
  const [enableStaking, setEnableStaking] = useState(true);
  const [stakingDuration, setStakingDuration] = useState<'30' | '90' | '180' | '365'>('90');
  const [lockPeriod, setLockPeriod] = useState<number>(90); // days
    ...
    <div className="space-y-6">
      ...
      {activeTab === 'setup' && (
        <div className="space-y-6">
          ...
          {/* Step 4.5: Staking & Safety */}
          {selectedProvider && selectedModel && datasetHash && dataSize && (
            <div className="bg-gray-800/50 rounded-lg p-6 border border-gray-700">
              <div className="flex items-center mb-4">
                <div className="w-8 h-8 rounded-full bg-yellow-600 text-white flex items-center justify-center font-bold mr-3">4.5</div>
                <h3 className="text-xl font-bold">Staking & Model Safety</h3>
              </div>
              <div className="space-y-4">
                <div className="bg-yellow-900/20 border border-yellow-500/30 rounded-lg p-4">
                  <div className="flex items-start mb-2">
                    ...
                    <div>
                      <div className="text-yellow-400 font-semibold mb-1">Model Safety Staking</div>
                      <div className="text-sm text-gray-300">
                        Stake tokens to ensure model safety and earn yield. Higher staking increases model trust and provides additional security.
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between p-4 bg-black/50 rounded-lg border border-gray-700">
                  <div>
                    <div className="text-sm text-gray-400 mb-1">Enable Staking</div>
                    <div className="text-xs text-gray-500">Stake tokens for model safety verification</div>
                  </div>
                  ...
                  <>
                    <div>
                      ...
                      <input
                        ...
                        min="0"
                        step="0.1"
                        className="w-full bg-black/50 border border-gray-700 rounded-lg px-4 py-3 text-white placeholder-gray-400 focus:border-yellow-500 focus:outline-none"
                      />
                      <div className="flex gap-2 mt-2">
                        <button
                          onClick={() => setStakeAmount((parseFloat(balance) * 0.1).toFixed(2))}
                          className="text-xs bg-gray-700 hover:bg-gray-600 text-white px-3 py-1 rounded transition-colors"
                        >
                          10%
                        </button>
                        <button
                          onClick={() => setStakeAmount((parseFloat(balance) * 0.25).toFixed(2))}
                          className="text-xs bg-gray-700 hover:bg-gray-600 text-white px-3 py-1 rounded transition-colors"
                        >
                          25%
                        </button>
                        <button
                          onClick={() => setStakeAmount((parseFloat(balance) * 0.5).toFixed(2))}
                          className="text-xs bg-gray-700 hover:bg-gray-600 text-white px-3 py-1 rounded transition-colors"
                        >
                          50%
                        </button>
                        <button
                          onClick={() => setStakeAmount((parseFloat(balance) * 0.75).toFixed(2))}
                          className="text-xs bg-gray-700 hover:bg-gray-600 text-white px-3 py-1 rounded transition-colors"
                        >
                          75%
                        </button>
                    ...
                    <div>
                      ...
                      <div className="text-xs text-gray-400 mt-2">
                        Lock period: {lockPeriod} days • Longer lock = Higher yield
                      </div>
                    </div>

                    {/* Yield Calculator */}
                    <div className="bg-black/50 rounded-lg p-4 border border-gray-700">
                      <div className="text-sm font-semibold text-gray-300 mb-3">Yield Calculator</div>
                      <div className="space-y-2">
                        <div className="flex justify-between items-center">
                          <span className="text-sm text-gray-400">Staking Amount:</span>
                          <span className="text-white font-medium">{stakeAmount || '0'} OG</span>
                        </div>
                        <div className="flex justify-between items-center">
                          <span className="text-sm text-gray-400">APY Rate:</span>
                          <span className="text-yellow-400 font-medium">{yieldRate}%</span>
                        </div>
                        <div className="flex justify-between items-center">
                        ...
                        <div className="border-t border-gray-700 pt-2 mt-2">
                          <div className="flex justify-between items-center">
                            <span className="text-sm font-semibold text-gray-300">Estimated Yield:</span>
                            <span className="text-green-400 font-bold text-lg">
                              {stakeAmount ? (parseFloat(stakeAmount) * (parseFloat(yieldRate) / 100) * (lockPeriod / 365)).toFixed(4) : '0'} OG
                            </span>
                          </div>
                          <div className="text-xs text-gray-500 mt-1">
                            Earned over {lockPeriod} days
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    {(parseFloat(stakedAmount) > 0 || parseFloat(yieldEarned) > 0) && (
                      <div className="bg-green-900/20 border border-green-500/30 rounded-lg p-4">
                        <div className="text-sm font-semibold text-green-400 mb-2">Current Staking Status</div>
                        <div className="space-y-2 text-sm">
                          <div className="flex justify-between">
                            <span className="text-gray-400">Staked Amount:</span>
                            <span className="text-white">{stakedAmount} OG</span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-gray-400">Yield Earned:</span>
                            <span className="text-green-400">{yieldEarned} OG</span>
                          </div>
                          <div className="flex justify-between">
                            <span className="text-gray-400">Remaining Lock:</span>
                    ...
                    <button
                      onClick={async () => {
                        ...
                        try {
                          setLoading(true);
                          setError('');
                          // Mock staking - in production, this would call the staking contract
                          await new Promise(resolve => setTimeout(resolve, 1500));
                          setStakedAmount(stakeAmount);
                          setYieldEarned((parseFloat(stakeAmount) * (parseFloat(yieldRate) / 100) * (lockPeriod / 365)).toFixed(4));
                          setSuccess(`Successfully staked ${stakeAmount} OG for ${lockPeriod} days!`);
                        } catch (err) {
                          setError('Failed to stake: ' + (err as Error).message);
                        } finally {
                          setLoading(false);
                        }
                      }}
                      disabled={loading || !stakeAmount || parseFloat(stakeAmount) <= 0}
                      className="w-full bg-yellow-600 hover:bg-yellow-700 text-white px-6 py-3 rounded-lg font-semibold transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
                    >
                      {loading ? (
                        <span className="flex items-center justify-center">
                        ...
                        `🔒 Stake ${stakeAmount || '0'} OG for Model Safety`
                      )}
                    </button>
                  </>
                )}
              ...
              <div className="space-y-4">
                <div className="bg-black/50 rounded-lg p-4 border border-gray-700">
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    {enableStaking && parseFloat(stakedAmount) > 0 && (
                      <>
                        <div className="border-t border-gray-700 pt-2 mt-2"></div>
                        <div className="flex justify-between">
                          <span className="text-gray-400">🔒 Staked for Safety:</span>
                          <span className="text-yellow-400 font-semibold">{stakedAmount} OG</span>
                        </div>
                        <div className="flex justify-between">
